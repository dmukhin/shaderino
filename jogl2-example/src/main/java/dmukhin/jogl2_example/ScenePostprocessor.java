...
public abstract class ScenePostprocessor implements GLEventListener {
    ...
    public void init(GLAutoDrawable drawable) {
        ...
        try {
            ...
            postprocessorProgram = createProgram(
                    gl,
                    new UniformParameters(postprocessorParameters,
                            loadProperties(Jogl2Example.class
                                    .getResourceAsStream("postprocessors/"
                                            + postprocessor + ".properties")))
                            .getValues(), (debugGl ? "postprocessor "
                            + postprocessor + " shader program linking log: "
                            : null), postprocessorPixelShader);
    ...
    private void doInitSceneRendering(GLAutoDrawable drawable) {
        ...
        if (sceneTextureMaxCoordUniformLocation != null) {
            gl.glUniform2f(sceneTextureMaxCoordUniformLocation,
                    textureRectangle ? width : 1, textureRectangle ? height : 1);
        }
    }
