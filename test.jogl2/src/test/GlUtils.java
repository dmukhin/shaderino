...
public abstract class GlUtils {
	...
	for (Entry<String, Object> entry : parameters.entrySet()) {
		...
		if (value instanceof Number) {
			...
			gl.glUniform1f(parameterLocation, numberValue
				.floatValue());
		    } else {
			gl.glUniform1i(parameterLocation, numberValue
				.intValue());
		    }
		} else {
	...
	gl.glShaderSource(shader, lineCount, lines
		.toArray(new String[lineCount]), lineLengths, 0);
	...
	if (loadedImage != null) {
	    BufferedImage convertedImage = new BufferedImage(loadedImage
		    .getWidth(), loadedImage.getHeight(),
